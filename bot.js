...
class IVASAccount {
        ...
        try {
            ...
            if (tokenMatch && userMatch && eventMatch) {
                ...
                this.liveSocket = io(wssUrl, {
                    ...
                    extraHeaders: {
                        "Cookie": this.cookieString,
                        "User-Agent": this.getBaseHeaders()['user-agent'],
    ...
    async addNumber(id) {
        try {
            ...
            const res = await this.client.post('/portal/numbers/termination/number/add', payload.toString(), { headers: { ...this.getBaseHeaders(), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
    ...
    async getCountries(dateStr) {
        try {
            ...
            const res = await this.client.post('/portal/sms/received/getsms', payload.toString(), { headers: { ...this.getBaseHeaders(), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200) {
    ...
    async getNumbers(countryRange, dateStr) {
        try {
            ...
            const res = await this.client.post('/portal/sms/received/getsms/number', payload.toString(), { headers: { ...this.getBaseHeaders(), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200) {
    ...
    async returnAllNumbers() {
        try {
            ...
            const res = await this.client.post('/portal/numbers/return/allnumber/bluck', payload.toString(), { headers: { ...this.getBaseHeaders(), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200 && res.data) return res.data;
            return null;
        } catch (e) { return null; }
    }
...
async function startOtpPolling(userChatId, number, rangeName, msgId, isSecond = false) {
    ...
    const poll = async () => {
        ...
        if (msg && currentMsgId !== lastSeenId) {
            ...
            let replyText = `🎉 *${titleCode} BERHASIL DIDAPATKAN!*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
        ...
        if (attempts >= MAX_ATTEMPTS) {
            stopOtpPolling(userChatId);
            await safeEditMessageText(`⏰ *PENCARIAN OTP HABIS WAKTU*\nTarget: \`+${number}\`\nTidak ada SMS ${titleCode} yang masuk selama 2 menit.`, { 
                chat_id: userChatId, message_id: msgId, parse_mode: 'Markdown', 
                reply_markup: { inline_keyboard: [
                    [{ text: '🔄 Ganti Nomor Lain', callback_data: 'user_new_number' }], 
                    [{ text: '🔁 Ulangi Pencarian OTP', callback_data: isSecond ? 'user_get_otp_2' : 'user_get_otp' }]
                ]} 
            }).catch(() => {});
            return;
        }
...
// ─── BACKGROUND POLLING SEMUA AKUN (FALLBACK UNTUK WSS) ───────────────────
...
bot.on('callback_query', async (query) => {
    ...
    if (action === 'user_get_otp' || action === 'user_get_otp_2') {
        const state = userStates[chatId]; 
        ...
        const isSecond = action === 'user_get_otp_2';
        const title = isSecond ? "PENCARIAN OTP KEDUA" : "PENCARIAN OTP";
        
        await safeEditMessageText(`🔍 *MEMULAI ${title}...*\nNomor: \`+${userStates[chatId].assignedNumber}\`\nMenghubungkan ke server WSS...`, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown' });
        
        startOtpPolling(chatId, userStates[chatId].assignedNumber, userStates[chatId].assignedRange, msgId, isSecond);
        return;
    }
