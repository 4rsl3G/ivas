...
const POLLING_INTERVAL = 20000; // 20 Detik
...
const getMainMenuMarkup = () => ({
    inline_keyboard: [
        ...
        [{ text: '🗑 Reset Database', callback_data: 'cmd_delete_all' }, { text: '⚙️ Status Server OTP', callback_data: 'cmd_status' }],
        [{ text: '🔍 Cari Riwayat OTP', callback_data: 'cmd_search' }, { text: '👥 Daftar Pengguna', callback_data: 'cmd_manage_users' }],
        [{ text: '🏦 Kelola Akun API List', callback_data: 'cmd_list_accounts' }]
    ]
});
...
// ─── CORE: KELAS IVAS ACCOUNT DENGAN CYCLE TLS ─────────────────────────────
class IVASAccount {
    ...
    async request(endpoint, options = {}) {
        ...
        const res = await cycleTlsInstance(url, {
            headers: reqHeaders,
            ja3: this.ja3,
            userAgent: userAgent,
    ...
    async initSession() {
        try {
            const res = await this.request('/portal/sms/received', { headers: { 'Accept': 'text/html' }});
            if (res.status === 200) {
    ...
    async getMyNumbers() {
        try {
            ...
            const res = await this.request(`/portal/numbers?${params.toString()}`);
    ...
    async getTerminationDetails(id) {
        try {
            ...
            const res = await this.request('/portal/numbers/termination/details', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
    ...
    async addNumber(id) {
        try {
            ...
            const res = await this.request('/portal/numbers/termination/number/add', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
    ...
    async getCountries(dateStr) {
        try {
            ...
            const res = await this.request('/portal/sms/received/getsms', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200) {
    ...
    async getNumbers(countryRange, dateStr) {
        try {
            ...
            const res = await this.request('/portal/sms/received/getsms/number', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200) {
    ...
    async getMessages(phoneNumber, countryRange, dateStr) {
        try {
            ...
            const res = await this.request('/portal/sms/received/getsms/number/sms', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200) {
    ...
    async returnAllNumbers() {
        try {
            ...
            const res = await this.request('/portal/numbers/return/allnumber/bluck', { method: 'POST', body: payload.toString(), headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } });
            if (res.status === 200 && res.data) return res.data;
            return null;
        } catch (e) { return null; }
    }
...
async function startOtpPolling(userChatId, number, rangeName, msgId) {
    ...
    const poll = async () => {
        attempts++; const elapsed = attempts * 20;
        ...
        const timeoutId = setTimeout(poll, 20000); 
...
bot.on('callback_query', async (query) => {
    ...
    if (action === 'dummy_btn') {
        return bot.answerCallbackQuery(query.id, { text: '💡 Ketuk teks angka OTP (abu-abu) di dalam pesan untuk menyalin otomatis.', show_alert: true });
    }
    ...
    if (action === 'user_get_number' || action === 'user_new_number') {
        if (action === 'user_new_number') { 
            ...
            stopOtpPolling(chatId); await releaseNumberFromUser(chatId); 
        }
        if (getActiveAccounts().length === 0) {
            return safeEditMessageText("⚠️ *SISTEM OFFLINE*\nInfrastruktur API sedang tidak terhubung.", { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: isAdmin(chatId) ? getMainMenuMarkup() : getUserMenuMarkup() });
        }
        ...
        if (!assigned) {
            return safeEditMessageText("❌ *STOK HABIS*\nSeluruh nomor WhatsApp sedang digunakan oleh pengguna lain.", { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔄 Coba Cari Lagi', callback_data: 'user_get_number' }]] } });
        }
        ...
        await safeEditMessageText(
            ...
            { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: { inline_keyboard: [ [{ text: `📱 +${assigned.number}`, callback_data: 'dummy_btn' }], [{ text: '📨 Cek SMS OTP (Mulai Pantau)', callback_data: 'user_get_otp' }], [{ text: '🔄 Ganti Nomor Lain', callback_data: 'user_new_number' }] ]} }
        );
        return;
    }
    ...
    if (action === 'user_get_otp') {
        ...
        if (!number || !rangeName) {
            ...
            if (!assigned) return safeEditMessageText("❌ *SESI TIDAK VALID*\nSesi nomor Anda telah berakhir atau belum mengambil nomor.", { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: isAdmin(chatId) ? getMainMenuMarkup() : getUserMenuMarkup() });
    ...
    if (action === 'cmd_list_accounts') {
        ...
        const inline_keyboard = accounts.map(acc => [{ text: `🗑 Hapus Akun ID ${acc.id} (${acc.label || 'No Label'})`, callback_data: `cmd_del_account_${acc.id}` }]);
    ...
    if (action.startsWith('cmd_del_account_')) {
        ...
        return safeEditMessageText(`⚠️ *KONFIRMASI HAPUS AKUN*\nAkun API ID: \`${accId}\` akan dihapus permanen beserta seluruh nomornya.`, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '⚠️ YA, HAPUS AKUN INI', callback_data: `cmd_confirm_del_account_${accId}` }], [{ text: '❌ Batal', callback_data: 'cmd_list_accounts' }]] }});
    }
    ...
    if (action === 'cmd_broadcast_ranges') {
        ...
        } catch (e) { return safeEditMessageText(`❌ *GAGAL MENYIARKAN*\nPastikan bot adalah Admin di Kanal.`, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: getMainMenuMarkup() }); }
    }

    if (action === 'cmd_login') {
        userStates[chatId].state = 'WAITING_HEADERS'; 
        userStates[chatId].tempLabel = '';
        return safeEditMessageText("🔑 *TAMBAH AKUN API IVAS*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\nKirimkan teks berformat JSON berisi *Full Headers* dari request browser Anda untuk mencegah deteksi Cloudflare.\n\nContoh:\n`{\n  \"Cookie\": \"cf_clearance=...; ivas_sms_session=...\",\n  \"User-Agent\": \"...\"\n}`", { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: getCancelMarkup() });
    } 
    ...
    if (action === 'cmd_sync_db') {
        ...
        for (const [accId, acc] of activeAccs) {
            ...
            if (myNumbers.length > 0) { await autoSaveNumbers(chatId, myNumbers, msgId, accId); totalSynced += myNumbers.length; }
        }
    ...
    if (action.startsWith('add_term_')) {
        ...
        if (result && result.message) {
            ...
            if (resStr.includes('done') || resStr.includes('success') || resStr.includes('berhasil')) {
                ...
                if (newNumbers.length > 0) await autoSaveNumbers(chatId, newNumbers, msgId, accId);
    ...
    if (action === 'cmd_hunt_wa') {
        ...
        if (purchasedRanges.length > 0) {
            ...
            await safeEditMessageText(reply + `⏳ _Sinkronisasi Basis Data..._`, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown' });
            ...
            for (const accId of successfulAccIds) {
                ...
                if (newNumbers.length > 0) {
                    await delay(1000);
                    await autoSaveNumbers(chatId, newNumbers, msgId, accId);
                }
            }
            safeEditMessageText(reply + `*🔥 PANEL ADMIN WHATSAPP OTP*`, { chat_id: chatId, message_id: msgId, parse_mode: 'Markdown', reply_markup: getMainMenuMarkup() }).catch(()=>{});
        } else {
    ...
    if (action === 'cmd_delete_all') {
        return safeEditMessageText("⚠️ *KONFIRMASI RESET DATABASE*\nSeluruh nomor WA dari semua akun akan dikembalikan ke server (Purge).", { 
            ...
            reply_markup: { inline_keyboard: [ [{ text: '⚠️ YA, KOSONGKAN SEMUA', callback_data: 'cmd_confirm_delete_all' }], [{ text: '❌ Batal', callback_data: 'cmd_cancel' }] ]} 
        });
    }
    ...
    if (action === 'cmd_status') {
        ...
        let statusMsg = `⚙️ *STATUS SERVER WHATSAPP OTP*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
...
bot.on('message', async (msg) => {
    ...
    if (currentState === 'WAITING_HEADERS') {
        try {
            ...
            const account = new IVASAccount(accountId, headersObj);
            if (await account.initSession()) {
                activeSessions.set(accountId, account);
                const myNumbers = await account.getMyNumbers();
                if (myNumbers.length > 0) await autoSaveNumbers(chatId, myNumbers, menuMsgId, accountId);
                ...
                safeEditMessageText("❌ *GAGAL AUTENTIKASI*\nHeaders tidak valid, terblokir Cloudflare, atau expired. Akun tidak disimpan.", { chat_id: chatId, message_id: menuMsgId, parse_mode: 'Markdown', reply_markup: getMainMenuMarkup() });
            }
            userStates[chatId].state = 'IDLE';
        } catch (e) {
            safeEditMessageText(`❌ *FORMAT SALAH*\nPastikan format teks adalah JSON yang valid.\n\n_Pesan Error: ${e.message}_`, { chat_id: chatId, message_id: menuMsgId, parse_mode: 'Markdown', reply_markup: getCancelMarkup() });
        }
    } 
        ...
        try {
            ...
            if (foundNumbers.length > 0) {
                ...
                const inline_keyboard = foundNumbers.slice(0, 10).map((n) => [{ text: `📱 +${n.number} ($${n.rate}) - Akun ${n.accId}`, callback_data: `term_detail_${n.id}_${n.accId}` }]);
...
// ─── INIT ──────────────────────────────────────────────────────────────────
(async () => {
    console.log('[SISTEM] Menyiapkan Basis Data WHATSAPP OTP...');
    ...
    for (const accData of accounts) {
        try {
            ...
            if (await account.initSession()) {
                ...
                console.log(`[API] Akun ID ${accData.id} GAGAL (Sesi mati/Terblokir).`);
            }
        } catch (e) {
            console.log(`[API] Akun ID ${accData.id} memiliki format data yang rusak.`);
        }
    }
    console.log(`[SISTEM] ${loadedCount}/${accounts.length} Akun API aktif.`);
    pollAllAccounts();
    console.log('[SISTEM] Bot WHATSAPP OTP siap melayani.');
})();
