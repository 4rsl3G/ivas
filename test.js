...
bot.on('callback_query', async (query) => {
    ...
    if (action === 'cmd_status') {
        ...
        let statusMsg = `⚙️ *STATUS SERVER FIX MERAH*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    if (currentState === 'WAITING_HEADERS') {
        try {
            ...
            if (await account.initSession()) {
                ...
                if (myNumbers.length > 0) await autoFilterAndSaveNumbers(chatId, myNumbers, menuMsgId, accountId);
...
bot.on('document', async (msg) => {
    ...
    try {
        ...
        const sm = await bot.sendMessage(chatId, `📁 *FILE TXT DITERIMA*\n━━━━━━━━━━━━━━━━━━━━━━━━━━\nTotal: *${uniqueNumbers.length}* Nomor Telepon.\n\nPilih kecepatan ekstraksi:`, { 
            parse_mode: 'Markdown', 
            reply_markup: { inline_keyboard: [[ { text: '🚀 Turbo', callback_data: `start_${jobId}_fast` }, { text: '🚗 Normal', callback_data: `start_${jobId}_normal` }, { text: '🚲 Siluman', callback_data: `start_${jobId}_slow` } ]]} 
        });
        if (userStates[chatId]) userStates[chatId].lastMsgId = sm.message_id;
    } catch (e) { 
...
// ─── INIT ──────────────────────────────────────────────────────────────────
(async () => {
    console.log('[SISTEM] Menyiapkan Basis Data FIX MERAH...');
    ...
    console.log('[SISTEM] Bot FIX MERAH siap melayani.');
})();
